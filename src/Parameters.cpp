...
#ifdef ESP8266
void Parameters::handleWebPage(ESP8266WebServer &http, const char *path) {
  String page;
  ...
  if (http.method() == HTTP_GET) {
    ...
    page.concat(F("</head>\n"
      "<body>\n"
      "<form action=\""));
    page.concat(FPSTR(path));
    page.concat(F("\" method=\"post\">\n"
      "<table cols=2>\n"));
...
#else
void Parameters::handleWebPage(WebServer &http, const char *path) {
  String page;
...
bool paramsCaptivePortal(Parameters *params, const char *ssid, const char *pswd, uint16_t duration, int8_t ledPin, bool ledLevel) {
#ifdef ESP8266
  ...
  http->on(FPSTR(SLASH_PSTR), [&]() {
    params->handleWebPage(*http, SLASH_PSTR);
#else
  http->on(SLASH_PSTR, [&]() {
    params->handleWebPage(*http, SLASH_PSTR);
#endif
  ...
  if (duration) {
    ...
    while (millis() - start < duration * 1000) {
      blinkFast = WiFi.softAPgetStationNum() > 0;
      if (blinkFast)
        start = millis();
      dns->processNextRequest();
      http->handleClient();
      if (ledPin >= 0) {
        digitalWrite(ledPin, (millis() % (blinkFast ? 250 : 500) < 25) == ledLevel);
      }
      delay(1);
    }
  } else {
