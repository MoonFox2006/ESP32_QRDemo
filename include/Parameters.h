...
#define PARAM_IP_CUSTOM(n, t, d1, d2, d3, d4, e) { .name = (n), .title = (t), .type = paraminfo_t::PARAM_IP, .size = (sizeof(uint8_t) * 4), .defvalue = { .asip = { (d1), (d2), (d3), (d4) }  }, .editor = e }
...
#ifdef ESP8266
  void handleWebPage(ESP8266WebServer &http, const char *path);
#else
  void handleWebPage(WebServer &http, const char *path);
#endif
...
bool paramsCaptivePortal(Parameters *params, const char *ssid, const char *pswd, uint16_t duration = 0, int8_t ledPin = -1, bool ledLevel = LOW);
